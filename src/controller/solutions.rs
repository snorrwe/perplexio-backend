...
pub fn get_solution_by_game_id(
    ...
    let result = get_users_solutions(&db_client(&config), &current_user, game_id);
    Ok(Json(result))
}
...
pub fn submit_solutions(
    ...
    let current_solutions = get_users_solutions(&transaction, &current_user, game_id);
